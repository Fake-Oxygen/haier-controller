...
typedef int (*set_heatpump_mode_t)(const struct device *dev, enum heatpump_mode mode);
typedef int (*set_heatpump_state_t)(const struct device *dev, enum heatpump_state state);

__subsystem struct heatpump_driver_api {
  heatpump_get_3way_t get_3way;
  read_float_t read_ch_temp;
  read_float_t read_dhw_temp;
  read_float_t read_dhw_target_temp;
  ...
  set_float_t set_dhw_temp;
  set_heatpump_mode_t set_mode;
  set_heatpump_state_t set_state;
};
...
static inline float heatpump_read_dhw_target_temp(const struct device *dev) {
  __ASSERT_NO_MSG(dev != NULL);
  return DEVICE_API_GET(heatpump, dev)->read_dhw_target_temp(dev);
}
...
static inline int heatpump_set_dhw_temp(const struct device *dev, float temp) {
  __ASSERT_NO_MSG(dev != NULL);
  return DEVICE_API_GET(heatpump, dev)->set_dhw_temp(dev, temp);
}

static inline int heatpump_set_mode(const struct device *dev, enum heatpump_mode mode) {
  __ASSERT_NO_MSG(dev != NULL);
  return DEVICE_API_GET(heatpump, dev)->set_mode(dev, mode);
}

static inline int heatpump_set_state(const struct device *dev, enum heatpump_state state) {
  __ASSERT_NO_MSG(dev != NULL);
  return DEVICE_API_GET(heatpump, dev)->set_state(dev, state);
}
#endif 
